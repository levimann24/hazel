...
module.exports = class Cache {
  ...
  async refreshCache() {
    console.log('[hazel] Cache refresh triggered')
    const { account, repository, channel = 'production', token } = this.config
    ...
    // Look for releases based on channel
    let release
    if (channel === 'production') {
      // Production: latest stable release (not pre-release)
      release = data.find(item => !item.draft && !item.prerelease)
    } else if (channel === 'development') {
      // Development: latest pre-release
      release = data.find(item => !item.draft && item.prerelease)
    } else {
      // Fallback to old logic for backward compatibility
      const isPre = Boolean(this.config.pre) === Boolean(item.prerelease)
      release = data.find(item => !item.draft && isPre)
    }
